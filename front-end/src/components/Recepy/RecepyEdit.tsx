...
const RecepyEdit = (props: any) => {
    ...
    const addCollection = async (): Promise<void> => {
        ...
        const response = await fetch(`https://localhost:7247/recepy/${props.props}`, {
            headers: {
    ...
    return (
        <div>
            ...
            <Dialog open={open} onClose={handleClose}>
                <DialogTitle>Koreguoti receptą</DialogTitle>
                <DialogContent>
                    <DialogContentText>Koreguokite pavadinimą, aprašymą arba trukmę</DialogContentText>
                    <Box
                        component="form"
                        noValidate
                        autoComplete="off"
                    ...
                    >
                        ...
                        <div>
                            <TextField onChange={(e) => setDuration(e.target.value)} required id="outlined-password-input" label="Trukmė" type="text" />
                        </div>
                    </Box>
                </DialogContent>
                <DialogActions>
